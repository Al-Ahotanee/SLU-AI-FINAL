...
║                                                                      ║
║   AI Engine: HuggingFace Space (sentence-transformers)              ║
║   This server: Auth · Documents · Search · Admin · SPA              ║
╚══════════════════════════════════════════════════════════════════════╝
"""
...
AI_BASE   = os.environ.get("AI_ENGINE_URL", "").rstrip("/")   # HuggingFace Space URL
AI_SECRET = os.environ.get("AI_SECRET", "slu-ai-bridge-secret-change-me")
AI_TIMEOUT = int(os.environ.get("AI_TIMEOUT", 15))            # seconds

def _ai_post(path: str, payload: dict) -> dict | None:
    """
    POST to the HuggingFace AI engine.
    Returns parsed JSON dict or None if AI is unavailable (graceful degradation).
    """
    if not AI_BASE:
        return None
    url = f"{AI_BASE}{path}"
    ...
    req  = urllib.request.Request(
        ...
        headers={
            ...
            "Authorization": f"Bearer {AI_SECRET}",
        },
        method="POST",
    )
    try:
        ...
        log.warning(f"[AI] {path} failed: {e}")
        return None

def _ai_get_embedding(text: str) -> list | None:
    res = _ai_post("/embed", {"text": text[:5000]})
    return res["embedding"] if res else None

def _ai_classify(text: str, filename: str) -> dict:
    res = _ai_post("/classify", {"text": text[:5000], "filename": filename})
    if res:
        return res
    # local keyword fallback
    return _local_classify(text, filename)

def _ai_extract_tags(text: str, n: int = 12) -> list:
    res = _ai_post("/tags", {"text": text[:5000], "n": n})
    if res:
        return res["tags"]
    return _local_tags(text, n)

def _ai_search(query: str, docs: list) -> list:
    """
    Send slim doc payload to AI engine for scoring.
    docs: list of dicts with id, title, description, course_code,
          academic_level, resource_type, tags, text_snippet, embedding.
    Returns list of {id, relevance} dicts, sorted by relevance desc.
    """
    res = _ai_post("/search", {"query": query, "docs": docs})
    if res:
        return res["results"]
    # local TF-IDF fallback
    return _local_tfidf_search(query, docs)

def _ai_recommend(target: dict, candidates: list, n: int = 6) -> list:
    res = _ai_post("/recommend", {"target": target, "candidates": candidates, "n": n})
    if res:
        return res["results"]
    return _local_tfidf_recommend(target, candidates, n)
...
# ── text extraction (local — no heavy model needed) ───────────────────────────
...
# ── LOCAL fallback AI (keyword-only, no numpy) ────────────────────────────────
COURSE_KW = {
...
def _local_classify(text, fname):
    t = (text + " " + fname).lower()
...
def _local_tfidf_search(query, docs):
    ql = query.lower()
    corpus = [f"{d.get('title','')} {d.get('description','')} {d.get('text_snippet','')}" for d in docs]
    if not corpus: return []
    vecs = _tfidf(corpus + [query]); qv = vecs[-1]; scored = []
    for i,d in enumerate(docs):
        sim = _cos_dict(vecs[i], qv)
        if ql in str(d.get("title","")).lower(): sim += 0.5
        if sim > 0.01: scored.append({"id":d["id"],"relevance":round(sim,4)})
    return sorted(scored, key=lambda x: x["relevance"], reverse=True)[:50]

def _local_tfidf_recommend(target, candidates, n=6):
    corpus = [f"{c.get('title','')} {c.get('description','')} {c.get('text_snippet','')}" for c in candidates]
    tt = f"{target.get('title','')} {target.get('description','')} {target.get('text_snippet','')}"
    vecs = _tfidf(corpus + [tt]); tv = vecs[-1]; scores = []
    for i,c in enumerate(candidates):
        sim = _cos_dict(vecs[i], tv)
        if c.get("course_code") and c["course_code"] == target.get("course_code"): sim += 0.3
        scores.append((c["id"], sim))
    scores.sort(key=lambda x: x[1], reverse=True)
    return [{"id":cid,"relevance":round(s,4)} for cid,s in scores[:n]]

# ── AI pipeline helpers used by routes ───────────────────────────────────────
def process_upload_ai(title, desc, text, filename, provided_cc, provided_al, provided_rt):
    """
    Run classify + tags + embed via AI engine (or local fallback).
    Returns (course_code, academic_level, resource_type, tags, embedding_json)
    """
    # Classification
    if not (provided_cc and provided_al and provided_rt):
        cls = _ai_classify(text, filename)
        cc  = provided_cc or cls["course_code"]
        al  = provided_al or cls["academic_level"]
        rt  = provided_rt or cls["resource_type"]
    else:
        cc, al, rt = provided_cc, provided_al, provided_rt

    # Tags
    tags = _ai_extract_tags(f"{title} {desc} {text}")

    # Embedding
    emb  = _ai_get_embedding(f"{title} {desc} {text[:3000]}")
    emb_json = json.dumps(emb) if emb else ""

    return cc, al, rt, tags, emb_json

def _doc_to_search_payload(d: dict) -> dict:
    """Convert a DB row dict to the slim payload the AI /search expects."""
    emb = None
    raw = d.get("embedding","")
    if raw:
        try: emb = json.loads(raw)
        except: pass
    return {
        "id":             d["id"],
        "title":          d.get("title",""),
        ...
        "academic_level": d.get("academic_level",""),
        "resource_type":  d.get("resource_type",""),
        "tags":           d.get("tags",""),
        "text_snippet":   (d.get("text_content","") or "")[:1000],
        "embedding":      emb,
    }
...
def search_docs(query, filters, uid=None):
    ...
    # Build slim payload and ask AI engine to score
    payloads = [_doc_to_search_payload(d) for d in docs]
    scored   = _ai_search(query, payloads)   # [{id, relevance}, …]

    # Map scores back onto full doc dicts
    score_map = {item["id"]: item["relevance"] for item in scored}
    result = []
    for d in docs:
        if d["id"] in score_map:
            d["relevance"] = score_map[d["id"]]
            result.append(d)
    return sorted(result, key=lambda x: x["relevance"], reverse=True)[:50]
...
def get_recs(doc_id, n=6):
    ...
    target_slim = _slim(target)
    cands_slim  = [_slim(c) for c in cands]
    scored = _ai_recommend(target_slim, cands_slim, n)
    ...
    for d in cands:
        if d["id"] in score_map:
            d["relevance"] = score_map[d["id"]]
            result.append(d)
    return sorted(result, key=lambda x: x["relevance"], reverse=True)[:n]
...
def health():
    ...
    return jsonify({
        "status":    "ok",
        "ai_engine": AI_BASE or "local-fallback",
...
def upload_doc():
    ...
    al     = request.form.get("academic_level","")
    rt     = request.form.get("resource_type","")
    ay     = request.form.get("academic_year","")
    pub    = int(request.form.get("is_public",1))

    # Extract text locally (no heavy model needed)
    text = extract_text(dest, ext)

    # Delegate AI work to HuggingFace Space
    cc, al, rt, tags, emb_json = process_upload_ai(title, desc, text, file.filename, cc, al, rt)
    ...
    cur=db.execute(
        ...
        "VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?)",
        (title,desc,safe,file.filename,ext,size,cc,al,rt,ay,g.user_id,text,emb_json,pub))
    did=cur.lastrowid
...
if __name__ == "__main__":
    init_db()
    log.info(f"SLU Repo (Render) starting on port {PORT} | AI engine: {AI_BASE or 'local-fallback'}")
